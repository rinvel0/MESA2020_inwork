...
static void init_port_cu48_kba(meba_inst_t inst, mesa_port_no_t port_no, meba_port_entry_t *entry)
{
    ...
    if (port_no < 18 || port_no == 20) {
        ...
        //entry->map.miim_addr       = port_no == 18 ? 7 : (port_no == 19 ? 6 : (port_no == 21 ? 5 : (port_no == 22 ? 4 : (port_no == 23 ? 11 : (port_no == 24 ? 10 : (port_no == 25 ? 9 : 8))))));
        //entry->map.miim_addr       = port_no == 18 ? 8 : (port_no == 19 ? 9 : (port_no == 21 ? 10 : (port_no == 22 ? 11 : (port_no == 23 ? 4 : (port_no == 24 ? 5 : (port_no == 25 ? 9 : 8))))));
...
static void jr2_init_cu48(meba_inst_t inst)
{
    ...
    for (gpio_no = 56; gpio_no < 60; gpio_no++) {
        (void)mesa_gpio_mode_set(NULL, 0, gpio_no, MESA_GPIO_ALT_1); //CPMOD
    }
    
    for (gpio_no = 8; gpio_no < 13; gpio_no++) { //CPMOD GPIO Serial lines to alternate 1
...
static const board_func_t board_funcs[] = {
    ...
    [BOARD_TYPE_JAGUAR2_CU48] = {
        ...
        .init_port  = init_port_cu48_kba, //CMPOD was without kba
                            ...
                            const meba_board_interface_t *callouts)
{
    ...
    if ((inst = meba_state_alloc(callouts,
                                 "Jaguar-2",
                                 MESA_TARGET_SPARX_IV_80, // CPMOD maybe MESA_TARGET_SPARX_IV_80 was MESA_TARGET_JAGUAR_2
                                 sizeof(*board))) == NULL) {
        printf(" meba state alloc NULL ? ");
        return NULL;
    }
    ...
    if (meba_conf_get_hex(inst, "type", &i) == MESA_RC_OK) {
        ...
        board->type = BOARD_TYPE_JAGUAR2_CU48;//CPMOD was BOARD_TYPE_JAGUAR2;   // Default
    }
    ...
    board->type = BOARD_TYPE_JAGUAR2_CU48; //CPMOD hardcoded
    ...
    switch (board->type) {
        /*case BOARD_TYPE_SERVAL2_NID:
            board->port_cnt = 15;
            break;*/
        case BOARD_TYPE_JAGUAR2_CU48:                      //CPMOD was 90
            if (inst->props.target == MESA_TARGET_SPARX_IV_80 || inst->props.target == MESA_TARGET_SPARX_IV_52) {
                board->port_cnt = 53;
                //inst->props.mux_mode = MESA_PORT_MUX_MODE_2; //maybe try another MUX MODE? not documented //CPMOD
            } else {
                board->port_cnt = 53;  //CPMOD was 51
            }
